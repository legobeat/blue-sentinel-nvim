...
function sha1_to_str(bytes)
  -- Show the SHA1 hash as a hex string
  local str = ""
  for _,b in ipairs(bytes) do
            ...
            if string.match(http_data, "\r\n\r\n$") then
              ...
              for line in vim.gsplit(http_data, "\r\n") do
                ...
                elseif string.match(line, nocase("Sec%-WebSocket%-Key")) then
                  _, kstart = string.find(line, "Key:%s*")
                  _, kend = string.find(line, "Key:%s*%S+")
                  websocketkey = string.sub(line, kstart+1, kend)
                end
              end

              if has_upgrade then
                local magic = "258EAFA5-E914-47DA-95CA-C5AB0DC85B11" -- Websocket magic string
                local hash = sha1.sha1(websocketkey .. magic)
                local encoded_hash = base64.encode(hash)

                sock:write("HTTP/1.1 101 Switching Protocols\r\n")
                sock:write("Upgrade: websocket\r\n")
                sock:write("Connection: Upgrade\r\n")
                sock:write("Sec-WebSocket-Accept: " .. encoded_hash .. "\r\n")
